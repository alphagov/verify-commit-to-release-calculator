...
def compute_difference(repo_name)
  ...
  shas.each do |sha|
    ...
    unless release_tag.empty?
      ...
      release_date = `find ../verify-release-logs -maxdepth 1 -type d -execdir git grep -i -h -e "#{parsed_repo_name}" --and -e '"#{release_tag_number}"' --and -e "Send release complete email" \\; | jq ".end_time"`.chomp
